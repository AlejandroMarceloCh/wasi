...
const DistrictMap = ({ onGo }) => {
  ...
  useE(() => {
    if (!elRef.current || mapRef.current || !window.L || distritos.length === 0) return;
    const map = L.map(elRef.current, {
    ...
    return () => {
      ...
      map.remove(); mapRef.current = null; markersRef.current = {}; lastFocusRef.current = null;
    };
  }, [distritos]);
...
const DashboardScreen = ({ role, onGo, onOpenAnalysis, onPublish, onError, onAuthExpired }) => {
  const isSeller = role === 'Propietario' || role === 'Agente inmobiliario';
